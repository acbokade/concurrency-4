...
void CreateGround(b2World& World,float X, float Y)
{
    ...
    Body->SetUserData(&num1);
}

int i=0;
struct timeval current_time,prev_time;
queue<pair<int,int>> Q;
void collide(int a,int b,int i)
{
        
        if(a==1 || b==1)
        {
            cout<<"Collision no : "<<cnt<<" Thread no: "<<i<<endl;
            cnt++;
        }
...
void checkcollision()
{
    ...
    for(int i=0;i<r1;i++)
    {
        ...
        Q.pop();
        worker[i]=thread(collide,p.first,p.second,i);
    }
    for(int i=0;i<r1;i++)
        worker[i].join();
}
class myListener : public b2ContactListener
{
    void BeginContact(b2Contact* contact)
    {
        ...
        {    
            ...
            Q.push({*(int *)a->GetUserData(),*(int *)b->GetUserData()});
        }
    }
};
...
int main()
{
    ...
    //creating circle
    b2BodyDef bodydef1;
    bodydef1.position=b2Vec2(200.f/SCALE,200.f/SCALE);
    ...
    //creating body
    b2BodyDef bodydef2;
    bodydef2.position=b2Vec2(200.f/SCALE,270.f/SCALE);
    bodydef2.type = b2_dynamicBody;
    body2 = world.CreateBody(&bodydef2);
    b2PolygonShape rect;
    rect.SetAsBox((20.f/2)/SCALE, (90.f/2)/SCALE);
    ...
    float temp = (37.5)/sqrt(2);
    float temp1 = 30/sqrt(2);
    //creating joint betweem head and body
    b2RevoluteJointDef jointDef;
    jointDef.bodyA= body1;
    jointDef.bodyB= body2;
    jointDef.collideConnected = false;
    jointDef.localAnchorA.Set(0.f,25.0f/SCALE);//the top right corner of the box
    jointDef.localAnchorB.Set(0.f,-45.0f/SCALE);//center of the circle*/
    ...
    //creating legs
    b2BodyDef bodydef3;
    bodydef3.position=b2Vec2((200.f-temp)/SCALE,(315.f+temp)/SCALE);
    bodydef3.type = b2_dynamicBody;
    body3 = world.CreateBody(&bodydef3);
    b2PolygonShape rect1;
    rect1.SetAsBox((10.f/2)/SCALE, (75.f/2)/SCALE);
    ...
    //creating legs
    b2BodyDef bodydef4;
    bodydef4.position=b2Vec2((200.f+temp)/SCALE,(315.f+temp)/SCALE);
    bodydef4.type = b2_dynamicBody;
    body4 = world.CreateBody(&bodydef4);
    b2PolygonShape rect2;
    rect2.SetAsBox((10.f/2)/SCALE, (75.f/2)/SCALE);
    ...
    //creating hands
    b2BodyDef bodydef5;
    bodydef5.position=b2Vec2((200.f-temp1)/SCALE,(225.f+temp1)/SCALE);
    bodydef5.type = b2_dynamicBody;
    body5 = world.CreateBody(&bodydef5);
    b2PolygonShape rect3;
    rect3.SetAsBox((10.f/2)/SCALE, (60.f/2)/SCALE);
    ...
    //creating hands
    b2BodyDef bodydef6;
    bodydef6.position=b2Vec2((200.f+temp1)/SCALE,(225.f+temp1)/SCALE);
    bodydef6.type = b2_dynamicBody;
    body6 = world.CreateBody(&bodydef6);
    b2PolygonShape rect4;
    rect4.SetAsBox((10.f/2)/SCALE, (60.f/2)/SCALE);
    ...
    jointDef1.localAnchorA.Set(0.f/SCALE,45.0f/SCALE);//the top right corner of the box
    jointDef1.localAnchorB.Set((temp-12.f)/SCALE,-temp/SCALE);//center of the circle
    //jointDef1.referenceAngle=0.0;
    jointDef1.enableLimit = true;
    jointDef1.enableMotor=false;
    //jointDef1.collideConnected = true;
    jointDef1.lowerAngle = 35 * DEGTORAD;
    jointDef1.upperAngle = 55 * DEGTORAD;
    ...
    jointDef2.localAnchorA.Set(0.f/SCALE,45.0f/SCALE);//the top right corner of the box
    jointDef2.localAnchorB.Set(-(temp-12.f)/SCALE,-temp/SCALE);//center of the circle
    jointDef2.enableLimit = true;
    //jointDef2.referenceAngle=0.0;
    jointDef2.enableMotor=false;
    //jointDef2.collideConnected = true;
    jointDef2.lowerAngle = -55 * DEGTORAD;
    jointDef2.upperAngle = -35 * DEGTORAD;
    ...
    jointDef3.localAnchorA.Set(0.f/SCALE,-45.0f/SCALE);//the top right corner of the box
    jointDef3.localAnchorB.Set((temp1-7.f)/SCALE,-temp1/SCALE);//center of the circle
    jointDef3.referenceAngle=0.0;
    jointDef3.enableLimit = true;
    jointDef3.enableMotor=false;
    jointDef3.lowerAngle = 20 * DEGTORAD;
    jointDef3.upperAngle = 70 * DEGTORAD;
    ...
    jointDef4.localAnchorA.Set(0.f/SCALE,-45.0f/SCALE);//the top right corner of the box
    jointDef4.localAnchorB.Set(-(temp1-7.f)/SCALE,-temp1/SCALE);//center of the circle
    jointDef4.referenceAngle=0.0;
    jointDef4.enableMotor=false;
    jointDef4.enableLimit = true;
    jointDef4.lowerAngle = -70 * DEGTORAD;
    jointDef4.upperAngle = -20 * DEGTORAD;
    b2RevoluteJoint* joint4 = (b2RevoluteJoint*)world.CreateJoint(&jointDef4);

    body1->SetUserData(&num2);
    body2->SetUserData(&num2);
    body3->SetUserData(&num2);
    body4->SetUserData(&num2);
    body5->SetUserData(&num2);
    body6->SetUserData(&num2);

    sf::CircleShape shape(25.f);
    shape.setFillColor(sf::Color(0, 250, 0));

    sf::RectangleShape recta(sf::Vector2f(20, 90));
    recta.setFillColor(sf::Color(150, 0, 0));

    sf::RectangleShape recta1(sf::Vector2f(10, 75));
    recta1.setFillColor(sf::Color(0, 0, 150));

    sf::RectangleShape recta2(sf::Vector2f(10, 75));
    recta2.setFillColor(sf::Color(0, 0, 150));

    sf::RectangleShape recta3(sf::Vector2f(10, 60));
    recta3.setFillColor(sf::Color(0, 0, 150));

    sf::RectangleShape recta4(sf::Vector2f(10, 60));
    ...
    while (window.isOpen())
    {
        ...
        double t = (double) ((current_time.tv_sec * 1000000 + current_time.tv_usec) - (prev_time.tv_sec * 1000000 + prev_time.tv_usec)) / 1000.0;
